...
const DashboardUsuario = () => {
  ...
  // 🔹 Ahora la donación se solicita al backend
  const realizarDonacion = async (monto) => {
    ...
    try {
      ...
      const response = await fetch("/api/wompi/donacion", {
        method: "POST",
      ...
      if (!response.ok) {
        setErrorDonacion("Error creando donación en el servidor");
        return;
      }
      ...
      setErrorDonacion("No se pudo iniciar el pago.");
    }
  };
  // -----------------------------------------------------
